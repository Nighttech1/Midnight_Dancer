...
- Flutter SDK 3.0+
- Dart 3.0+
- Android SDK: `D:\Applications\Android\Sdk` — в `android/local.properties`, скриптах и (после одного запуска) в профиле PowerShell + среде пользователя: `.\scripts\setup_android_home_profile.ps1`. Debug на телефоне: `.\scripts\run_android_debug.ps1 -Flavor full` (см. `BUILD.md`).
...
3. Запуск: в Cursor/VS Code выбери конфигурацию Run (Lite / Standard / Full / English). Или из терминала **из папки Midnight_Dancer**:
   ```bash
